...
export default function ModalPostularse(props) {
    const { 
        oferta, 
        onClose = () => {}, 
    ...
    // Store answers in state as { [preguntaId]: "SI" | "NO" }
    ...
    useEffect(() => {
        ...
        style.innerHTML = `
            ...
            .chamba-modal-submit-btn:hover {
                opacity: 0.95 !important;
            }
            .chamba-modal-submit-btn:active {
    ...
    const handleFileChange = (e) => {
        ...
        if (file) {
            ...
            // Simulate file upload (mock upload with Drive link mapping to bypass Backend regex checker)
            setTimeout(() => {
    ...
    const handleSubmit = async (e) => {
        ...
        // Validate PDF upload if native CV is not checked
        if (!usarCvNativo && !cvUrl) {
            setError("Por favor, subí tu archivo CV en formato PDF.")
            return
        }
        ...
        for (const question of filterQuestions) {
            if (!respuestas[question.id]) {
                setError("Por favor, respondé a todas las preguntas de filtro obligatorias.")
                return
            }
        }
        ...
        setLoading(true)
        try {
            const mappedAnswers = Object.keys(respuestas).map(key => ({
                preguntaId: key,
                respuestaDada: respuestas[key]
            }))
            ...
            await fetchConAuth("/postulaciones", {
                method: "POST",
        ...
        <div style={backdropStyle}>
            ...
            >
                ...
                <form onSubmit={handleSubmit} style={formScrollableBody}>
                    ...
                    {/* Checkbox: Use Native CV */}
                    <label style={checkboxContainerStyle}>
                        <div 
                            onClick={() => setUsarCvNativo(!usarCvNativo)}
                            style={customCheckbox(usarCvNativo, primaryColor)}
                        >
                    ...
                    <div 
                        ...
                        style={{
                            ...uploaderBoxStyle,
                            opacity: usarCvNativo ? 0.5 : 1,
                            cursor: usarCvNativo ? "not-allowed" : "pointer"
                        }}
                    >
                        <input
                            type="file"
                    ...
                    <div style={footerWrapper}>
                        <button type="button" onClick={onClose} className="chamba-modal-cancel" style={cancelBtnStyle}>
                            Cancelar
                        </button>
                        <button 
                            type="submit" 
...
const checkboxLabelTextWrapper = {
    display: "flex",
    flexDirection: "column",
    gap: "2px"
}
...
const uploaderMainText = {
    fontSize: "14px",
    fontWeight: "700",
    color: "#334155"
}

const uploaderSubText = {
    fontSize: "12.5px",
    color: "#64748B",
    fontWeight: "500"
}

const uploaderDetailsText = {
    fontSize: "11px",
    color: "#94A3B8",
    fontWeight: "500"
}
